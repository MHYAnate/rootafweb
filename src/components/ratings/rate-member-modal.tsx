...
export function RateMemberModal({
  ...
  // Logged in but not a verified client
  const canRate =
  ...
  const defaultTrigger = (
    <Button variant="outline" className="rounded-xl gap-2 font-medium hover:bg-amber-50 hover:text-amber-900 hover:border-amber-200 dark:hover:bg-amber-900/20 dark:hover:border-amber-800 transition-colors">
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      {/*
       * max-h-[85dvh] ensures it leaves safe space on mobile devices.
       * flex & flex-col creates a rigid container where the header is fixed
       * and only the inner div (overflow-y-auto) scrolls.
       */}
