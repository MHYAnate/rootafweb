...
// ── Must match Prisma enums (same as new page) ──
const CONDITION_OPTIONS = [
...
export default function EditToolPage() {
  const { id } = useParams();
  const router = useRouter();
  const { data: response, isLoading } = useTool(id as string);
  const { mutate: updateTool, isPending } = useUpdateTool();
  const [images, setImages] = useState<any[]>([]);
  const [tags, setTags] = useState<string[]>([]);
  const [tagInput, setTagInput] = useState('');
  ...
  // response = { data: Tool } from toolsApi.getById()
  ...
  useEffect(() => {
    ...
    reset({
      ...
      modelNumber: t.modelNumber || '',
      quantityAvailable: t.quantityAvailable ?? 1,
      listingPurpose: t.listingPurpose || 'FOR_SALE',
      salePricingType: t.salePricingType || 'FIXED',
      salePrice: t.salePrice ? Number(t.salePrice) : undefined,
      leasePricingType: t.leasePricingType || 'FIXED',
      leaseRate: t.leaseRate ? Number(t.leaseRate) : undefined,
      leaseRatePeriod: t.leaseRatePeriod || 'PER_DAY',
      depositRequired: t.depositRequired || 'NOT_REQUIRED',
      depositAmount: t.depositAmount ? Number(t.depositAmount) : undefined,
      pickupLocation: t.pickupLocation || '',
      pickupLocationState: t.pickupLocationState || '',
      pickupLocationLga: t.pickupLocationLga || '',
      deliveryAvailable: t.deliveryAvailable || false,
      deliveryFee: t.deliveryFee ? Number(t.deliveryFee) : undefined,
  ...
  const onSubmit = (formData: ToolFormData) => {
    const isSale =
      formData.listingPurpose === 'FOR_SALE' ||
      formData.listingPurpose === 'BOTH';
    const isLease =
      formData.listingPurpose === 'FOR_LEASE' ||
      formData.listingPurpose === 'BOTH';
    ...
    const payload: Record<string, any> = {
      ...
      quantityAvailable: formData.quantityAvailable || 1,
      tags,
    ...
    if (formData.pickupLocationState)
      payload.pickupLocationState = formData.pickupLocationState;
    if (formData.pickupLocationLga)
      payload.pickupLocationLga = formData.pickupLocationLga;

    // Sale pricing
    if (isSale) {
      payload.salePricingType = formData.salePricingType || 'FIXED';
    ...
    // Lease pricing
    if (isLease) {
      payload.leasePricingType = formData.leasePricingType || 'FIXED';
      payload.leaseRatePeriod = formData.leaseRatePeriod || 'PER_DAY';
      if (formData.leaseRate && formData.leaseRate > 0) {
        payload.leaseRate = formData.leaseRate;
      }
      payload.depositRequired = formData.depositRequired || 'NOT_REQUIRED';
      if (
        formData.depositRequired === 'REQUIRED' &&
    ...
    if (formData.deliveryAvailable) {
      if (formData.deliveryFee && formData.deliveryFee > 0) {
        payload.deliveryFee = formData.deliveryFee;
      }
    ...
    updateTool(
      {
        ...
        data: {
          ...payload,
          images: images.map((img, idx) => ({
            // Keep existing image ids so the hook skips re-uploading them
            ...(img.id ? { id: img.id } : {}),
            imageUrl: img.imageUrl || img.url,
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => {
                    ...
                    setTimeout(() => {
                      trigger(['salePrice', 'leaseRate', 'depositAmount']);
                    }, 100);
                  }}
                >
                  <SelectTrigger className="h-11 rounded-lg">
