...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      reset({
        ...
        priceAmount: p.priceAmount ? Number(p.priceAmount) : undefined,
        availability: p.availability,
      });
      setImages(p.images || []);
      setOriginalImageIds(
        (p.images || []).map((img: any) => img.id).filter(Boolean),
      );
    }
  }, [data, reset]);
  ...
  const onSubmit = (formData: ProductFormData) => {
    // Images that are new (uploaded during this edit session, no `id` from DB)
    ...
    // The hook will:
    // 1. Update product fields (without images)
    // 2. Add any new images via POST /products/:id/images
    updateProduct(
      {
        id: id as string,
        ...
        <Card className="card-premium">
          ...
          <CardContent>
            <PricingInput
              ...
              amount={watch('priceAmount')}
              onAmountChange={(v) => setValue('priceAmount', v)}
            />
          </CardContent>
        </Card>
        <Card className="card-premium">
